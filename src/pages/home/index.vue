    ...
    <scroll-view>
      <div class="restaurantsList">
        <div
          ...
          :key="index"
          @click="toIndex(item.id,item.managerId,item.accountId)"
        >
          <div class="restaurants-info-image">
            <image :src="item.img_url" class="restaurants-image" />
          </div>
          <div class="restaurants-info">
            <div class="restaurants-info-name">{{ item.storename }}</div>
            <div class="restaurants-info-rating">
              <star :size="24" :score="item.score"></star>
              <div class="restaurants-info-rating-sales">
                月售 {{ item.sales }}
              </div>
            </div>
            <div class="restaurants-info-price">
              起送 {{ item.initial_price }}￥ | 配送
              {{ item.delivery_charge }}
            </div>
          </div>
          <div class="restaurants-distribution">
            <span class="restaurants-distribution-time"
              >{{ item.time }} 分钟</span
            >
            <span class="restaurants-distribution-distance">
              {{ item.distance }}</span
            >
          </div>
        </div>
